...
import pygame
import sys 
from src.constants import (ROWS, COLS, SQUARE_SIZE, BOARD_WIDTH, BOARD_HEIGHT, SIDE_PANEL_WIDTH, # INFO_PANEL_HEIGHT removed
                           WIDTH, HEIGHT, 
...
from src.assets_manager import get_piece_image
...
class Board:
    ...
    def _setup_buttons(self):
        ...
        first_button_y = status_text_y_center + 30 

        self.restart_button = Button(panel_x, first_button_y, button_width, button_height,
                                     text="Restart Game", action=self._handle_restart_click)
        self.buttons.append(self.restart_button)

        self.exit_button = Button(panel_x, self.restart_button.rect.bottom + spacing, button_width, button_height,
        ...
        self.confirm_no_button = Button(BOARD_WIDTH // 2 + 10, confirm_btn_y, 100, 40, "No",
                                        ...
                                        text_color=BUTTON_TEXT_COLOR,
                                        action=self._cancel_restart_confirmation)
    ...
    def handle_click(self, pos):
        if self.show_restart_confirmation:
            if self.confirm_yes_button.rect.collidepoint(pos):
                if self.confirm_yes_button.action: self.confirm_yes_button.action()
                return
    ...
    def select_square(self, row, col):
        ...
        if self.selected_square_coords is None:
            ...
            if (row, col) in self.valid_moves_coords:
                ...
                self.selected_square_coords = (row, col) if (row,col) != self.selected_square_coords else None
                self.valid_moves_coords = []
                if self.selected_square_coords: self._calculate_valid_moves(row, col)
            else:
                self.selected_square_coords = None
                self.valid_moves_coords = []
    ...
    def _update_status_message(self):
        if self.game_over: return
        turn = "White's Turn" if self.chess_board.turn == chess.WHITE else "Black's Turn"
        self.status_message = f"{turn}{' - CHECK!' if self.chess_board.is_check() else ''}"
    ...
    def draw_side_panel(self, screen):
        ...
        if not self.game_over and self.status_message:
    ...
    def draw_restart_confirmation_dialog(self, screen):
        if self.show_restart_confirmation:
            ...
            yes_color = self.confirm_yes_button.hover_color if yes_hover else self.confirm_yes_button.color
            pygame.draw.rect(overlay_surface, yes_color, yes_btn_rect_local, border_radius=5)
            yes_text = self.confirm_font.render("Yes", True, self.confirm_yes_button.text_color)
            ...
            no_color = self.confirm_no_button.hover_color if no_hover else self.confirm_no_button.color
            pygame.draw.rect(overlay_surface, no_color, no_btn_rect_local, border_radius=5)
            no_text = self.confirm_font.render("No", True, self.confirm_no_button.text_color)
